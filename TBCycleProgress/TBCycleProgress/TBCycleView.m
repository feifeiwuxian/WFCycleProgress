...
{
    if (_imgBg == nil) {
        _imgBg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"device_state_round_default"]];
        _imgBg.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
        [self.layer addSublayer:_imgBg.layer];
    }
...
//    _progressLayer.lineCap = kCALineCapRound;//指定线的边缘是圆的
    _progressLayer.lineWidth = 35;//线的宽度
...
//    [_gradientLayer addSublayer:rightLayer];
    ...
    // 设置为图片layer
    UIImageView *image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"device_state_round_blue"]];
    image.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
    [_gradientLayer addSublayer:image.layer];
...
- (void)startLayerAnimation{
    ...
    _progressLayer.strokeStart = 0.f;   // 设置起点为 0
    _progressLayer.strokeEnd = _startProgress;     // 设置终点为 0
    
    CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"strokeEnd"];
    animation.duration = 1.0 * fabs(_progress - _startProgress);   // 持续时间
    animation.fromValue = @(_startProgress); // 从 0 开始
    animation.toValue = @(_progress);   // 到 1 结束
    // 保持动画结束时的状态
    animation.removedOnCompletion = NO;
    animation.fillMode = kCAFillModeForwards;
    // 动画缓慢的进入，中间加速，然后减速的到达目的地。
    animation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionDefault];
    [_progressLayer addAnimation:animation forKey:@"mask"];
